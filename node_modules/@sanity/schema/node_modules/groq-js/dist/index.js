...
var debug__default = /* @__PURE__ */ _interopDefaultCompat(debug), __defProp$6 = Object.defineProperty, __defNormalProp$6 = (obj, key, value) => key in obj ? __defProp$6(obj, key, { enumerable: !0, configurable: !0, writable: !0, value }) : obj[key] = value, __publicField$6 = (obj, key, value) => (__defNormalProp$6(obj, typeof key != "symbol" ? key + "" : key, value), value);
...
function handleArrayCoerceNode(node, scope) {
  const base = walk({ node: node.base, scope });
  return $trace("arrayCoerce.base %O", base), mapUnion(base, (base2) => base2.type !== "array" ? { type: "null" } : base2);
}
function handleFlatMap(node, scope) {
  const base = walk({ node: node.base, scope });
  return mapUnion(base, (base2) => base2.type !== "array" ? base2 : walk({ node: node.expr, scope: scope.createHidden([base2.of]) }));
}
function handleMap(node, scope) {
  const base = walk({ node: node.base, scope });
  return $trace("map.base %O", base), mapUnion(base, (base2) => base2.type !== "array" ? base2 : maybeResolveInline(base2.of, scope, (base3) => {
    if (base3.type === "union") {
      const value = walk({ node: node.expr, scope: scope.createHidden(base3.of) });
      return $trace("map.expr %O", value), {
        type: "array",
        of: value
      };
    }
    if (base3.type === "object") {
      const value = walk({ node: node.expr, scope: scope.createHidden([base3]) });
      return $trace("map.expr %O", value), {
        type: "array",
        of: value
      };
...
function mapProjectionInScope(base, scope, mapper) {
  return base.type === "union" ? base.of.length === 1 ? mapProjectionInScope(base.of[0], scope, mapper) : {
    type: "union",
    of: base.of.map((node) => mapProjectionInScope(node, scope, mapper))
  } : base.type === "array" ? mapper(base.of) : mapper(base);
}
function handleProjectionNode(node, scope) {
  const base = walk({ node: node.base, scope });
  return $trace("projection.base %O", base), base.type === "unknown" || base.type === "null" ? { type: "null" } : mapProjectionInScope(base, scope, (field) => field.type === "null" || field.type === "unknown" ? { type: "null" } : walk({
    node: node.expr,
...
function mergeInlineObject(dst, src) {
  return {
    type: "object",
...
function mapFieldInScope(field, scope, mapper) {
  if (field.type === "union")
    return {
      type: "union",
...
function handleAccessAttributeNode(node, scope) {
  ...
  return node.base && (attributeBase = walk({ node: node.base, scope })), $trace("accessAttribute.base %s %O", node.name, attributeBase), mapFieldInScope(attributeBase, scope, (base) => {
    const attribute = base.attributes[node.name];
    return attribute !== void 0 ? ($debug(`accessAttribute.attribute found ${node.name} %O`, attribute), attribute.optional ? nullUnion(attribute.value) : attribute.value) : ($warn(
...
function handleAccessElementNode(node, scope) {
  if (!node.base)
    return { type: "unknown" };
  const base = walk({ node: node.base, scope });
  return $trace("accessElement.base %O", base), mapUnion(base, (base2) => base2.type !== "array" ? { type: "null" } : {
    type: "union",
    of: [base2.of, { type: "null" }]
  });
}
...
function handleSlice(node, scope) {
  ...
  const base = walk({ node: node.base, scope });
  return mapUnion(base, (base2) => base2.type !== "array" ? { type: "null" } : base2);
}
...
function maybeResolveInline(node, scope, mapper) {
  if (node.type === "inline") {
